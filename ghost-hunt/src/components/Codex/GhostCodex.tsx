...
const GHOST_DATA: Ghost[] = [
  ...
  },
  {
    ...
    name: 'PECCY',
    threatLevel: 'EXTREME',
    description: 'Appears harmless. Cute. Friendly. DO NOT BE FOOLED. ??????? ???????? ??????? This entity is EXTREMELY DANGEROUS despite its appearance. ??????? Investigators report feeling safe before ??? ???????? ??????? Multiple casualties. ??????? DO NOT APPROACH. ??????? Trust your equipment, not your eyes. ???????',
    emfReaction: '??? EXTREME ??? (100+ mG). All bars maxed. Constant red. Equipment malfunction likely. ???',
    thermalReaction: '??? ANOMALOUS ??? Temperature readings impossible. Freezing and burning simultaneously. Scanner glitches. ???',
    audioReaction: '??? CORRUPTED ??? Static. Whispers in unknown language. Equipment failure imminent. ???',
    cameraReaction: '??? DECEPTIVE ??? Photos show friendly appearance. Reality distortion detected. DO NOT TRUST VISUAL DATA. ???',
    identificationSteps: [
      '??? IDENTIFICATION IMPOSSIBLE ???',
      'All evidence triggers at once',
      'Equipment behaves erratically',
      'Visual appearance is DECEPTIVE',
      '??? IF ENCOUNTERED: EVACUATE IMMEDIATELY ???'
    ],
    weaknesses: ['??? UNKNOWN ???', '??? NO KNOWN COUNTERMEASURES ???', '??? AVOID AT ALL COSTS ???'],
    imageUrl: peccyImage,
    isLocked: false,
  },
  {
